...
{
    public enum AlgorithmEnum { AldousBroder, BinaryTree, Ellers, HuntAndKill, Kruskall, Prim, RecursiveDivision, Sidewinder, Wilson }
    ...
    static public class Wilson<T> where T : IComparable<T>
    {
        ...
        public static MazeGraph<T> Execute(MazeGraph<T> G)
        {
            ...
            while (unvisited.Count > 0)
            {
                ...
                path.Add(current);
                while (unvisited.IndexOf(current) != -1)
                {
    ...
    static public class Costs<T> where T : IComparable<T>
    {
        public static int[] Dijkstra(MazeGraph<T> g, int orig = 0)
        {            
            int n = g.NumVert;
            int v;
            int w = -1;
            bool[] visited = new bool[n];
            int[] costs = Enumerable.Repeat(int.MaxValue, n + 1).ToArray();
            costs[n] = -1;
            foreach (var neighbor in g.Adjacents(orig))
                costs[neighbor.vertex] = 1;
            costs[orig] = 0;
            visited[orig] = true;           
            for (var i = 1; i < n-1; ++i)
            {
                ...
                for (v = 0; v < n; v++)
                    if (!visited[v])
                    {
                        int costWV = g.hasEdge(w, v) ? 1 : int.MaxValue;
                        int Owv = SafeAdd(costs[w], costWV);
                        if (Owv < costs[v])
                        {
                            costs[v] = Owv;
                            costs[n] = Math.Max(costs[n],Owv);
                        }
                    }
            }                    
            return costs;
        ...
        public static int[][] Floyd(MazeGraph<T> g)
        {
            int n = g.NumVert;
            ...
            for (var i = 0; i < n; ++i)
            {
                ...
                costs[i][i] = 0;
                foreach(var neighbor in g.Adjacents(i))
                    costs[i][neighbor.vertex] = 1;
            }
            costs[n] = new int[3];
            for (var k = 0; k < n; ++k)
                for (var i = 0; i < n; ++i)
                    for (var j = 0; j < n; ++j)
                    {
                        costs[i][j] = Math.Min(costs[i][j], SafeAdd(costs[i][k], costs[k][j]));
                        if (costs[n][0] < costs[i][j] && costs[i][j] != int.MaxValue)
                        {
                            costs[n][0] = costs[i][j];
                            costs[n][1] = i;
                            costs[n][2] = j;
